            ...
            <div class="flex flex-col lg:flex-row gap-8">
                ...
                <div class="lg:w-1/3">
                    <div class="aspect-w-2 aspect-h-3 rounded-lg overflow-hidden shadow-lg relative group">
                        ...
                        @if ($book->quantity > 0)
                            <div
                                class="absolute top-4 right-4 bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm font-medium">
                                Available
                            </div>
                        @else
                            <div
                                class="absolute top-4 right-4 bg-red-100 text-red-800 px-3 py-1 rounded-full text-sm font-medium">
                                Unavailable
                            </div>
                        @endif
                    </div>
                                ...
                                <div class="bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <div class="flex items-center">
                                        <div class="flex items-center">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <i
                                                    class="fas fa-star {{ $i <= round($book->average_rating) ? 'text-yellow-400' : 'text-gray-300' }} text-sm"></i>
                                            @endfor
                                        </div>
                                        <span
                                            class="ml-2 text-gray-900 font-medium">{{ number_format($book->average_rating, 1) }}</span>
                        ...
                        <div class="flex flex-col sm:flex-row gap-4">
                            @if ($book->quantity > 0 && auth()->check())
                                <a href="{{ route('book-loans.create', $book) }}"
                                    class="flex-1 inline-flex items-center justify-center px-6 py-3 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-colors duration-200">
                                    <i class="fas fa-book-reader mr-2"></i>
                                    Ajukan Pinjaman
                                </a>
                            @endif
                            <a href="{{ route('books.index') }}"
                                class="flex-1 inline-flex items-center justify-center px-6 py-3 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors duration-200">
                                <i class="fas fa-arrow-left mr-2"></i>
                                Back to Books
                            </a>
                        </div>
                        ...
                        <form action="{{ route('comments.store', $book) }}" method="POST"
                            ...
                            <div class="mb-4">
                                ...
                                <div class="flex items-center">
                                    @for ($i = 1; $i <= 5; $i++)
                                        <button type="button"
                                            class="text-2xl text-gray-300 hover:text-yellow-400 focus:outline-none transition-colors duration-200"
                                            onclick="setRating({{ $i }})">
                                            ★
                                        </button>
                                    @endfor
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="comment" id="comment" rows="3"
                                    ...
                                    required placeholder="Share your thoughts about this book..."></textarea>
                            </div>
                            <button type="submit"
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex items-center">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <i
                                                    class="fas fa-star {{ $i <= $comment->rating ? 'text-yellow-400' : 'text-gray-300' }} text-sm"></i>
                                            @endfor
                                        </div>
                                    </div>
                                </div>
